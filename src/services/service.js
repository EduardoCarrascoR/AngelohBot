...
const memeJaidefinichon = async() => {
    let meme, random = Math.floor(Math.random() * 10)
    ...
    const listResult = await page.evaluate(()=>{
        let result = [];
        document.querySelectorAll('div.repeat__post div.post_inner').forEach((value, index) => {
            if (index > 0) {
                const image = value.querySelector('div.image_post figure img');
                const postLink = value.querySelector('div.post_header div.post_comments_counter a');
...
const memeReddit = async() => {
    ...
    await  RedditImageFetcher.fetch({
        type: 'custom',
        total: 1,
        subreddit: ['DylanteroYT', 'dankmemes', 'Memes_Of_The_Dank', 'memes', 'okbuddyretard'],
      }).then(async(data) => {
          if (data[0] !== undefined && data[0].image !== Array.isArray(data[0].image) && data[0].postLink !== null && data[0].NSFW !== true) {
            meme = data[0];
        } else {
